...
- [x] A function to append one line (timestamp + message) to the error-log
  file from `paths.rs`
- [x] `run_silently<F>(f: F)` where `F: FnOnce() -> anyhow::Result<()>`:
...
- [x] Unit tests: closure returns `Ok(())` → no log entry written; closure
