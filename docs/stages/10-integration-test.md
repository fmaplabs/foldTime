...
- [ ] Create `tests/integration_init_hook.rs`
- [ ] `tempfile::tempdir()` + shelled `git init` to build a throwaway repo
- [ ] Locate the compiled binary via `env!("CARGO_BIN_EXE_foldtime")` — Cargo
  sets this automatically for integration tests, no `assert_cmd` needed
- [ ] Run `foldtime init` (and `--with-config`) against the throwaway repo via
  `std::process::Command`
- [ ] Assert `.git/hooks/post-commit` exists and is executable
- [ ] Make a real `git commit` in the throwaway repo — set `GIT_AUTHOR_*` /
  `GIT_COMMITTER_*` env vars on the `Command` so the test doesn't depend on
  your global git config being present
- [ ] Point the binary at a temp DB rather than your real `~/.foldtime` — this
  is why Stage 3 flagged an env var override; set it on the `Command` you use
  to invoke `foldtime`
- [ ] Assert, via a direct `rusqlite::Connection::open` on that temp DB, that
