...
- [x] Locate the compiled binary via `env!("CARGO_BIN_EXE_foldtime")` — Cargo
  sets this automatically for integration tests, no `assert_cmd` needed
- [x] Run `foldtime init` (and `--with-config`) against the throwaway repo via
  `std::process::Command`
...
- [x] Point the binary at a temp DB rather than your real `~/.foldtime` — this
